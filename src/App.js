...
class App extends Component {
	...
	getUser = async username => {
		this.setState({ loading: true });
		const res = await axios.get(
			`https://api.github.com/users/${username}?&client_id=${process.env.REACT_APP_GITFINDER_CLIENT_ID}&client_secret=${process.env.REACT_APP_GITFINDER_CLIENT_SECRET}`
		);

		this.setState({ user: res.data, loading: false });
	};

	// Get user repositories according to github api
	getUserRepos = async username => {
		this.setState({ loading: true });
		const res = await axios.get(
			`https://api.github.com/users/${username}/repos?per_page=5&sort=created:asc&client_id=${process.env.REACT_APP_GITFINDER_CLIENT_ID}&client_secret=${process.env.REACT_APP_GITFINDER_CLIENT_SECRET}`
		);
